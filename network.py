...
class LSTMCell(nn.Module):
    ...
    def __init__(self, input_size, hidden_size):
        ...
        self.conv = nn.Conv2d
        self.Wxf = torch.Tensor(input_size,hidden_size).cuda()
        self.Wxi = torch.Tensor(input_size,hidden_size).cuda()
        self.Wxc = torch.Tensor(input_size,hidden_size).cuda()
        self.Wxo = torch.Tensor(input_size,hidden_size).cuda()
        self.Whf = torch.Tensor(hidden_size,hidden_size).cuda()
        self.Whi = torch.Tensor(hidden_size,hidden_size).cuda()
        self.Whc = torch.Tensor(hidden_size,hidden_size).cuda()
        self.Who = torch.Tensor(hidden_size,hidden_size).cuda()
        self.bf = torch.Tensor(hidden_size).cuda()
        self.bi = torch.Tensor(hidden_size).cuda()
        self.bc = torch.Tensor(hidden_size).cuda()
        self.bo = torch.Tensor(hidden_size).cuda()

    def forward(self, x, hidden):
        # TODO:
        hn,cn = hidden
        f = self.sigmoid(torch.mm(x,self.Wxf)+torch.mm(hn,self.Whf)+self.bf)
        i = self.sigmoid(torch.mm(x,self.Wxi)+torch.mm(hn,self.Whi)+self.bi)
        C1 = self.tanh(torch.mm(x,self.Wxc)+torch.mm(hn,self.Whc)+self.bc)
        cn = f*cn + i*C1
        o = self.sigmoid(torch.mm(x,self.Wxo)+torch.mm(hn,self.Who)+self.bo)
        hn = o*self.tanh(cn)
        hidden = (hn,cn)
        return hidden
...
class Decoder(nn.Module):
    ...
    def forward(self, features, captions, lengths):
        ...
        # word embeddings from captions; (2) concatenate visual      (features, bx1xd) and extracted
        ...
        feature_cat = torch.cat((features.unsqueeze(1),word_embed),1)
        hiddens, states = self.lstm(feature_cat, states)
        outputs = self.linear(hiddens.squeeze(1))    # outputs: (batch_size, t+1, vocab_size)
